...
# EORTC QLQ scoring algorithm functions
from vantage6_strongaya_measures_licenced.proms.eortc_qlq_scoring import collect_variable_info, \
    compose_variable_details, orchestrate_scoring, ItemsToScoreInput


@data(1)
@algorithm_client
def partial_eortc_qlq_general_statistics(client: AlgorithmClient, df: pd.DataFrame, items_to_score: ItemsToScoreInput,
                                         variables_to_stratify: StratificationDetails = None) -> Dict[str, str]:
    """
    Execute the partial algorithm for EORTC QLQ C30 style scoring and general statistics computation.
    ...
    """
    safe_log("info",
             "Executing partial algorithm for EORTC QLQ C30 style scoring and general statistics computation thereof.")
...
@data(1)
@algorithm_client
def partial_eortc_qlq_aggregate_adjusted_deviation(client: AlgorithmClient, df: pd.DataFrame,
                                                   items_to_score: ItemsToScoreInput,
                                                   numerical_aggregated_results: Dict[str, str],
                                                   variables_to_stratify: StratificationDetails = None) -> dict[str, str]:
    """
    Execute the partial algorithm for EORTC QLQ C30 style scoring and aggregate-adjusted deviation computation.
    ...
    """
    safe_log("info",
             "Executing partial algorithm to compute EORTC QLQ C30 style scoring and aggregate adjusted deviation.")
